...
> **Status**: página dividida em **4 blocos**, um por NFR — ver [§0](#_0-divisão-do-foco-entre-os-4-membros). **Bloco A (Performance/Latência)**: v1 concluída (27/08/2026), pendente revisão da subequipe. **Blocos B, C e D**: pendentes.

## 0. Divisão do Foco entre os 4 Membros

A SubEquipe_01 tem 4 integrantes. Em vez de construir a Engenharia Reversa e o BPMN só em torno do softgoal oficialmente atribuído à subequipe (Performance/Latência — [Escopo do Produto §6](../../../Projeto/EscopoProduto.md#_6-requisitos-não-funcionais-candidatos-insumo-para-os-sigs)), o grupo decidiu levar adiante, até este foco, a mesma ideia que já havia aplicado no [SIG](2.NFRFramework.md#_3-softgoal-escolhido-e-justificativa): cada membro investiga a **mesma plataforma** pela lente de um NFR diferente. É **iniciativa extra** da subequipe — não substitui, e não deveria sobrepor, a divisão oficial entre as três subequipes do projeto ([Escopo do Produto §5](../../../Projeto/EscopoProduto.md#_5-fluxos-selecionados-para-engenharia-reversa--bpmn)).

| Bloco | NFR / Softgoal | Responsável | Nesta página | Em [BPMN](4.BPMN.md) | Pasta de evidências | Status |
| -- | -- | -- | -- | -- | -- | -- |
| **A** | Performance / Baixa Latência | Lucas Andrade Zanetti | [Bloco A](#bloco-a-performance-baixa-latência-lucas-andrade-zanetti) | D0–D5, D2b (prefixo `D`) | [`performance-latencia/`](../../../assets/engenharia-reversa/subequipe_01/performance-latencia/) | ✅ v1 concluída |
| **B** | Usabilidade / Interatividade | _(a definir)_ | [Bloco B](#bloco-b-usabilidade-interatividade-responsável-a-definir) | prefixo `U` | [`usabilidade-interatividade/`](../../../assets/engenharia-reversa/subequipe_01/usabilidade-interatividade/) | ⬜ pendente |
| **C** | Confiabilidade / Disponibilidade | _(a definir)_ | [Bloco C](#bloco-c-confiabilidade-disponibilidade-responsável-a-definir) | prefixo `C` | [`confiabilidade-disponibilidade/`](../../../assets/engenharia-reversa/subequipe_01/confiabilidade-disponibilidade/) | ⬜ pendente |
| **D** | Segurança | _(a definir)_ | [Bloco D](#bloco-d-segurança-responsável-a-definir) | prefixo `S` | [`seguranca/`](../../../assets/engenharia-reversa/subequipe_01/seguranca/) | ⬜ pendente |

> **Sobre a coluna Responsável.** Só o Bloco A está confirmado (é quem escreveu esta reestruturação). Os outros três seguem em aberto de propósito — esta página não atribui nome de colega sem confirmação em ata. Uma sugestão natural, dado que o SIG já tinha ramos por pessoa (Heitor → Segurança, Matheus → Confiabilidade e Usabilidade — [2.NFRFramework.md §1](2.NFRFramework.md#_1-metodologia-do-foco)), é reaproveitar essa afinidade agora que os 4 membros podem assumir 1 NFR cada; mas isso é proposta, a confirmar na próxima reunião da subequipe.

### 0.1. Por que dividir assim (e o que isso evita)

- **Paralelismo assíncrono real.** Cada bloco tem sua própria subseção nesta página, sua própria subpasta de evidências e seu próprio prefixo de diagrama (`D`/`U`/`C`/`S`). Um membro pode coletar, medir e escrever sem esperar os outros nem correr risco de sobrescrever o trabalho alheio — ver a mudança em [`coletar.sh`](https://github.com/UnBArqDsw2026-2-Turma01/G7_LiveVideoStreaming/blob/main/tools/engenharia-reversa/coletar.sh), que agora **exige** a subpasta do bloco como argumento.
- **Reaproveitamento sem acoplamento.** §1 (Metodologia) e §2 (Fundamentação Teórica) abaixo são **compartilhados pelos 4 blocos** — o nível de abstração (caixa-preta, protocolo), os limites éticos da coleta e a base teórica de engenharia reversa não mudam com o NFR. Cada bloco só precisa escrever o que é específico dele: contexto, hipóteses, processo, evidências, achados, rastreabilidade e senso crítico.
- **Menção superior, não redundância.** Já é o mesmo argumento usado no SIG ([2.NFRFramework.md §3](2.NFRFramework.md#_3-softgoal-escolhido-e-justificativa)): usar vários recursos do framework de qualidade é critério de menção superior, e aprofundar 4 NFRs em vez de 1 é isso aplicado à Engenharia Reversa e ao BPMN.
- **Atenção a sobreposição com achados já existentes.** O Bloco A já toca, tangencialmente, em continuidade de reprodução sob falha (D4 — token vencido, segmento indisponível, nó de borda fora do ar), mas sempre do **lado do cliente**. Se o Bloco C (Confiabilidade/Disponibilidade) for adiante, o ângulo com menos sobreposição é o **lado do servidor** — replicação de serviço, failover de PoP/CDN, degradação graciosa sob perda de banda do streamer (ligações diretas com [SIG · O15-O17](2.NFRFramework.md#_51-operacionalizações-propostas)). Da mesma forma, o Bloco D (Segurança) pode aprofundar a *decisão* de autorização que o Bloco A já mediu em [EV-01](../../../assets/engenharia-reversa/subequipe_01/performance-latencia/ev01-token.json) (o token, hoje lido pela lente de latência) sob a lente de ameaça — quem pode forjar, revogar ou reaproveitar esse token.
- **Registro formal.** Esta divisão está registrada como iniciativa extra em [1.3. Iniciativas Extras](../../1.3.IniciativasExtras.md).
...
| Reprodutibilidade | Toda a coleta está roteirizada em [`tools/engenharia-reversa/`](https://github.com/UnBArqDsw2026-2-Turma01/G7_LiveVideoStreaming/blob/main/tools/engenharia-reversa/coletar.sh). Qualquer membro repete a coleta com um comando e obtém um conjunto novo de evidências |
...
## Bloco A — Performance / Baixa Latência (Lucas Andrade Zanetti)

> As seções 3 a 10 abaixo são o conteúdo deste bloco — v1 concluída em 27/08/2026, pendente revisão de um segundo membro (§9). Índice completo dos 4 blocos em [§0](#_0-divisão-do-foco-entre-os-4-membros).
...
| H04 | **CDN** com **cache de borda** | <svg width="14" height="14" viewBox="0 0 16 16" style="vertical-align:-2px"><circle cx="8" cy="8" r="7" fill="#2e7d32"/><path d="M4.6 8.2 6.9 10.5 11.4 5.8" fill="none" stroke="#fff" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"/></svg> **Confirmada** | Segmentos servidos com `X-Cache: Hit` e `Age` entre 2 s e 26 s, de um PoP no Rio de Janeiro — [EV-04](https://github.com/UnBArqDsw2026-2-Turma01/G7_LiveVideoStreaming/blob/main/docs/assets/engenharia-reversa/subequipe_01/performance-latencia/ev04-tempos.txt), [EV-05](https://github.com/UnBArqDsw2026-2-Turma01/G7_LiveVideoStreaming/blob/main/docs/assets/engenharia-reversa/subequipe_01/performance-latencia/ev05-medicoes-de-latencia.txt) |
...
| H05 | Chat em tempo real sobre **Pub/Sub** em conexão persistente | <svg width="14" height="14" viewBox="0 0 16 16" style="vertical-align:-2px"><circle cx="8" cy="8" r="7" fill="#2e7d32"/><path d="M4.6 8.2 6.9 10.5 11.4 5.8" fill="none" stroke="#fff" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"/></svg> **Confirmada e detalhada** | Uma única conexão WebSocket multiplexando 36 tópicos por canal, com saudação declarando `keepaliveSec` e URL de recuperação — [EV-06](https://github.com/UnBArqDsw2026-2-Turma01/G7_LiveVideoStreaming/blob/main/docs/assets/engenharia-reversa/subequipe_01/performance-latencia/ev06-tempo-real.txt) |
...
| 3 | Isolar a chamada de decisão | *Onde o sistema autoriza a reprodução?* | Busca no corpo das requisições da API pelo nome da operação que precede o pedido de manifesto. A resposta traz o token e os motivos de restrição | [EV-01](https://github.com/UnBArqDsw2026-2-Turma01/G7_LiveVideoStreaming/blob/main/docs/assets/engenharia-reversa/subequipe_01/performance-latencia/ev01-token.json) |
...
| 5 | Reconstruir o pedido de manifesto | *O que o player faz com o token?* | Montagem da URL do manifesto mestre com o par (valor, assinatura) devolvido — [`montar_url.py`](https://github.com/UnBArqDsw2026-2-Turma01/G7_LiveVideoStreaming/blob/main/tools/engenharia-reversa/montar_url.py) | [EV-02](https://github.com/UnBArqDsw2026-2-Turma01/G7_LiveVideoStreaming/blob/main/docs/assets/engenharia-reversa/subequipe_01/performance-latencia/ev02-manifesto-mestre.m3u8) |
...
| 7 | Amostrar a janela ao vivo | *Qual é o atraso e de quanto em quanto tempo ele anda?* | Seis leituras do manifesto de mídia a cada 2 s, comparando o relógio local com o fim do segmento mais novo — [`medir_playlist.py`](https://github.com/UnBArqDsw2026-2-Turma01/G7_LiveVideoStreaming/blob/main/tools/engenharia-reversa/medir_playlist.py) | [EV-05](https://github.com/UnBArqDsw2026-2-Turma01/G7_LiveVideoStreaming/blob/main/docs/assets/engenharia-reversa/subequipe_01/performance-latencia/ev05-medicoes-de-latencia.txt) |
| 8 | Cronometrar cada salto | *Onde o tempo é gasto: em cálculo ou em transporte?* | `curl -w` com DNS, TCP, TLS, TTFB e total para os cinco endereços do fluxo | [EV-04](https://github.com/UnBArqDsw2026-2-Turma01/G7_LiveVideoStreaming/blob/main/docs/assets/engenharia-reversa/subequipe_01/performance-latencia/ev04-tempos.txt) |
...
| 11 | Instrumentar a camada de tempo real | *Como o chat e os eventos chegam?* | Envelope em torno de `WebSocket` e `fetch` no console, seguido de uma troca de canal dentro da própria aplicação, para observar assinatura e cancelamento | [EV-06](https://github.com/UnBArqDsw2026-2-Turma01/G7_LiveVideoStreaming/blob/main/docs/assets/engenharia-reversa/subequipe_01/performance-latencia/ev06-tempo-real.txt) |
| 12 | Anonimizar e versionar | *Como publicar sem violar a diretriz?* | [`anonimizar.py`](https://github.com/UnBArqDsw2026-2-Turma01/G7_LiveVideoStreaming/blob/main/tools/engenharia-reversa/anonimizar.py) sobre cada captura, com tabela de substituição publicada | [ANONIMIZACAO.md](../../../assets/engenharia-reversa/subequipe_01/ANONIMIZACAO.md) |
| 13 | Traduzir achados em processo | *O que disso é atividade, evento, decisão e ator?* | Cada achado de §6 recebeu um destino no modelo; a rastreabilidade elemento a elemento está em [BPMN §6](4.BPMN.md#_6-rastreabilidade--elos-com-outros-artefatos) | [4.BPMN.md](4.BPMN.md) |
...
Todas em [`docs/assets/engenharia-reversa/subequipe_01/`](https://github.com/UnBArqDsw2026-2-Turma01/G7_LiveVideoStreaming/tree/main/docs/assets/engenharia-reversa/subequipe_01), já anonimizadas.

| # | Arquivo | O que contém | O que sustenta |
| -- | -- | -- | -- |
| **EV-01** | [`ev01-token.json`](https://github.com/UnBArqDsw2026-2-Turma01/G7_LiveVideoStreaming/blob/main/docs/assets/engenharia-reversa/subequipe_01/performance-latencia/ev01-token.json) | Resposta do serviço de autorização de reprodução | A03, A04, RN01, RN02, RN03 |
| **EV-02** | [`ev02-manifesto-mestre.m3u8`](https://github.com/UnBArqDsw2026-2-Turma01/G7_LiveVideoStreaming/blob/main/docs/assets/engenharia-reversa/subequipe_01/performance-latencia/ev02-manifesto-mestre.m3u8) + [`ev02-cabecalhos.txt`](https://github.com/UnBArqDsw2026-2-Turma01/G7_LiveVideoStreaming/blob/main/docs/assets/engenharia-reversa/subequipe_01/performance-latencia/ev02-cabecalhos.txt) | Manifesto mestre com a grade de qualidades e o bloco de metadados de sessão | A05, A08, RN05, RN06, H02, H03 |
| **EV-03** | [`ev03-manifesto-de-midia.m3u8`](https://github.com/UnBArqDsw2026-2-Turma01/G7_LiveVideoStreaming/blob/main/docs/assets/engenharia-reversa/subequipe_01/performance-latencia/ev03-manifesto-de-midia.m3u8) | Janela deslizante de uma qualidade, com marcações de anúncio e descontinuidade | A06, A07, RN04, RN07 |
| **EV-04** | [`ev04-tempos.txt`](https://github.com/UnBArqDsw2026-2-Turma01/G7_LiveVideoStreaming/blob/main/docs/assets/engenharia-reversa/subequipe_01/performance-latencia/ev04-tempos.txt) | DNS, TCP, TLS, TTFB e total dos cinco saltos do fluxo | RN08, orçamento de latência de D0 |
| **EV-05** | [`ev05-medicoes-de-latencia.txt`](https://github.com/UnBArqDsw2026-2-Turma01/G7_LiveVideoStreaming/blob/main/docs/assets/engenharia-reversa/subequipe_01/performance-latencia/ev05-medicoes-de-latencia.txt) | Amostragem da janela ao vivo, entrega do segmento e estado do player | RN04, RN07, RN09, RN10 |
| **EV-06** | [`ev06-tempo-real.txt`](https://github.com/UnBArqDsw2026-2-Turma01/G7_LiveVideoStreaming/blob/main/docs/assets/engenharia-reversa/subequipe_01/performance-latencia/ev06-tempo-real.txt) | Conexão persistente, saudação, tópicos assinados e troca de canal | A09, A10, RN11, RN12 |
...
| -- | -- | -- |
| Lucas Andrade Zanetti | Definição do recorte e do protocolo de coleta; execução das etapas 1–13; construção das ferramentas de coleta e anonimização; redação desta página | _(preencher com o link do commit/PR)_ |
| Heitor Macedo Ricardo | _(preencher — revisão dos achados e da classificação de confiança)_ | |
| Matheus Lemes Amaral | _(preencher)_ | |
| Pedro Druck Montalvão Reis | _(preencher)_ | |

> <svg width="15" height="14" viewBox="0 0 16 16" style="vertical-align:-2px"><path fill="#c77700" d="M8 1 15.5 14.5H.5z"/><rect x="7.2" y="5.6" width="1.6" height="4.6" fill="#fff"/><rect x="7.2" y="11.4" width="1.6" height="1.6" fill="#fff"/></svg> **Pendências antes da entrega**: revisão dos achados por pelo menos um segundo membro; segunda coleta em horário e rede diferentes; correção de O04 no [SIG](2.NFRFramework.md).

## Bloco B — Usabilidade / Interatividade (responsável a definir)

> **Status**: ⬜ pendente. Este bloco segue o mesmo método do [Bloco A](#bloco-a-performance-baixa-latência-lucas-andrade-zanetti) — a metodologia (§1) e a fundamentação teórica (§2) no topo desta página já valem para aqui, sem precisar repeti-las. O que falta é específico deste NFR: contexto, hipóteses, processo, evidências, achados, rastreabilidade e senso crítico.
>
> **Antes de começar**: copie `tools/engenharia-reversa/alvo.env.exemplo` (se ainda não existir `alvo.env`) e rode a coleta apontando para a subpasta deste bloco — nunca para a de outro membro:
> ```bash
> ./tools/engenharia-reversa/coletar.sh <canal> docs/assets/engenharia-reversa/subequipe_01/usabilidade-interatividade
> ```

### B.1. Contexto da Aplicação Analisada

| Item | Valor |
| -- | -- |
| Fluxo(s) candidato(s) | notificação de canal ao vivo para seguidores; navegação/descoberta de conteúdo (busca, categorias); personalização do player (legendas, múltiplas faixas de idioma) — *sugestão inicial da divisão do foco, não vinculante; o responsável define o recorte final* |
| Por que este fluxo liga ao NFR | _a preencher_ |
| Perfil de usuário a usar na exploração | _a preencher_ |

### B.2. Hipóteses a Verificar

_a preencher — hipóteses herdadas do [Rich Picture](1.ArtefatoGeneralista.md) ou do [SIG](2.NFRFramework.md) que este bloco pode confirmar ou refutar, no mesmo formato de [§3.1 do Bloco A](#_31-hipóteses-herdadas-do-rich-picture-resultado-da-verificação)._

### B.3. Processo Aplicado — Etapas

_a preencher — mesmo roteiro de 13 etapas do [Bloco A §4](#_4-processo-aplicado-etapas), adaptado ao que este NFR exige observar._

### B.4. Evidências Coletadas

_a preencher — arquivos em [`usabilidade-interatividade/`](../../../assets/engenharia-reversa/subequipe_01/usabilidade-interatividade/), já anonimizados por [`anonimizar.py`](https://github.com/UnBArqDsw2026-2-Turma01/G7_LiveVideoStreaming/blob/main/tools/engenharia-reversa/anonimizar.py) antes de versionar._

### B.5. Achados

_a preencher — atores, atividades/decisões e regras de negócio inferidas, com grau de confiança (Alto/Médio/Baixo), no mesmo formato do [Bloco A §6](#_6-achados)._

### B.6. Rastreabilidade & Elos com Outros Artefatos

_a preencher — todo elemento do BPMN deste bloco (prefixo `U` em [4.BPMN.md](4.BPMN.md)) precisa apontar para um achado registrado aqui._

> **Atenção a sobreposição**: a moderação de chat/UGC é a extra oficial da SubEquipe_03 ([Escopo do Produto §5](../../../Projeto/EscopoProduto.md#_5-fluxos-selecionados-para-engenharia-reversa--bpmn)) — se este bloco tocar em chat, focar na *interação do espectador* (latência percebida, feedback visual), não em moderação, para não duplicar.

### B.7. Senso Crítico

_a preencher._

### B.8. Participantes & Comprobatórios

| Membro | Contribuição | Comprobatório (commit/PR) |
| -- | -- | -- |
| _(a definir)_ | | |

## Bloco C — Confiabilidade / Disponibilidade (responsável a definir)

> **Status**: ⬜ pendente. Este bloco segue o mesmo método do [Bloco A](#bloco-a-performance-baixa-latência-lucas-andrade-zanetti) — a metodologia (§1) e a fundamentação teórica (§2) no topo desta página já valem para aqui, sem precisar repeti-las. O que falta é específico deste NFR: contexto, hipóteses, processo, evidências, achados, rastreabilidade e senso crítico.
>
> **Antes de começar**: copie `tools/engenharia-reversa/alvo.env.exemplo` (se ainda não existir `alvo.env`) e rode a coleta apontando para a subpasta deste bloco — nunca para a de outro membro:
> ```bash
> ./tools/engenharia-reversa/coletar.sh <canal> docs/assets/engenharia-reversa/subequipe_01/confiabilidade-disponibilidade
> ```

### C.1. Contexto da Aplicação Analisada

| Item | Valor |
| -- | -- |
| Fluxo(s) candidato(s) | tolerância a falha de PoP/CDN (failover de borda); degradação graciosa sob queda de banda do streamer; replicação de microsserviços (auto-scaling) — *sugestão inicial da divisão do foco, não vinculante; o responsável define o recorte final* |
| Por que este fluxo liga ao NFR | _a preencher_ |
| Perfil de usuário a usar na exploração | _a preencher_ |

### C.2. Hipóteses a Verificar

_a preencher — hipóteses herdadas do [Rich Picture](1.ArtefatoGeneralista.md) ou do [SIG](2.NFRFramework.md) que este bloco pode confirmar ou refutar, no mesmo formato de [§3.1 do Bloco A](#_31-hipóteses-herdadas-do-rich-picture-resultado-da-verificação)._

### C.3. Processo Aplicado — Etapas

_a preencher — mesmo roteiro de 13 etapas do [Bloco A §4](#_4-processo-aplicado-etapas), adaptado ao que este NFR exige observar._

### C.4. Evidências Coletadas

_a preencher — arquivos em [`confiabilidade-disponibilidade/`](../../../assets/engenharia-reversa/subequipe_01/confiabilidade-disponibilidade/), já anonimizados por [`anonimizar.py`](https://github.com/UnBArqDsw2026-2-Turma01/G7_LiveVideoStreaming/blob/main/tools/engenharia-reversa/anonimizar.py) antes de versionar._

### C.5. Achados

_a preencher — atores, atividades/decisões e regras de negócio inferidas, com grau de confiança (Alto/Médio/Baixo), no mesmo formato do [Bloco A §6](#_6-achados)._

### C.6. Rastreabilidade & Elos com Outros Artefatos

_a preencher — todo elemento do BPMN deste bloco (prefixo `C` em [4.BPMN.md](4.BPMN.md)) precisa apontar para um achado registrado aqui._

> **Atenção a sobreposição**: o Bloco A já modela recuperação de falha do **lado do cliente** ([D4](4.BPMN.md#_37-d4-recuperar-a-reprodução-após-falha)); e Recuperação de Falha de Transmissão do lado do streamer é a extra oficial da SubEquipe_02 ([Escopo do Produto §5](../../../Projeto/EscopoProduto.md#_5-fluxos-selecionados-para-engenharia-reversa--bpmn)). O ângulo com menos sobreposição é a resiliência **do lado do servidor** (O16-O17 do [SIG](2.NFRFramework.md#_51-operacionalizações-propostas)), majoritariamente não observável por caixa-preta — ver como o Bloco A tratou isso em [D1](4.BPMN.md#_35-d1-iniciar-e-configurar-a-transmissão) antes de escolher o recorte.

### C.7. Senso Crítico

_a preencher._

### C.8. Participantes & Comprobatórios

| Membro | Contribuição | Comprobatório (commit/PR) |
| -- | -- | -- |
| _(a definir)_ | | |

## Bloco D — Segurança (responsável a definir)

> **Status**: ⬜ pendente. Este bloco segue o mesmo método do [Bloco A](#bloco-a-performance-baixa-latência-lucas-andrade-zanetti) — a metodologia (§1) e a fundamentação teórica (§2) no topo desta página já valem para aqui, sem precisar repeti-las. O que falta é específico deste NFR: contexto, hipóteses, processo, evidências, achados, rastreabilidade e senso crítico.
>
> **Antes de começar**: copie `tools/engenharia-reversa/alvo.env.exemplo` (se ainda não existir `alvo.env`) e rode a coleta apontando para a subpasta deste bloco — nunca para a de outro membro:
> ```bash
> ./tools/engenharia-reversa/coletar.sh <canal> docs/assets/engenharia-reversa/subequipe_01/seguranca
> ```

### D.1. Contexto da Aplicação Analisada

| Item | Valor |
| -- | -- |
| Fluxo(s) candidato(s) | ameaças ao token de reprodução (forjar, revogar, reaproveitar); proteção de conteúdo/DRM na entrega; validação de direitos do streamer na ingestão — *sugestão inicial da divisão do foco, não vinculante; o responsável define o recorte final* |
| Por que este fluxo liga ao NFR | _a preencher_ |
| Perfil de usuário a usar na exploração | _a preencher_ |

### D.2. Hipóteses a Verificar

_a preencher — hipóteses herdadas do [Rich Picture](1.ArtefatoGeneralista.md) ou do [SIG](2.NFRFramework.md) que este bloco pode confirmar ou refutar, no mesmo formato de [§3.1 do Bloco A](#_31-hipóteses-herdadas-do-rich-picture-resultado-da-verificação)._

### D.3. Processo Aplicado — Etapas

_a preencher — mesmo roteiro de 13 etapas do [Bloco A §4](#_4-processo-aplicado-etapas), adaptado ao que este NFR exige observar._

### D.4. Evidências Coletadas

_a preencher — arquivos em [`seguranca/`](../../../assets/engenharia-reversa/subequipe_01/seguranca/), já anonimizados por [`anonimizar.py`](https://github.com/UnBArqDsw2026-2-Turma01/G7_LiveVideoStreaming/blob/main/tools/engenharia-reversa/anonimizar.py) antes de versionar._

### D.5. Achados

_a preencher — atores, atividades/decisões e regras de negócio inferidas, com grau de confiança (Alto/Médio/Baixo), no mesmo formato do [Bloco A §6](#_6-achados)._

### D.6. Rastreabilidade & Elos com Outros Artefatos

_a preencher — todo elemento do BPMN deste bloco (prefixo `S` em [4.BPMN.md](4.BPMN.md)) precisa apontar para um achado registrado aqui._

> **Atenção a sobreposição**: Autenticação/Cadastro é a entrega mínima oficial da SubEquipe_03, e validação de direitos do streamer é a extra oficial dela também ([Escopo do Produto §5](../../../Projeto/EscopoProduto.md#_5-fluxos-selecionados-para-engenharia-reversa--bpmn)). O ângulo com menos sobreposição é a segurança **do token de reprodução já medido pelo Bloco A** ([EV-01](../../../assets/engenharia-reversa/subequipe_01/performance-latencia/ev01-token.json)) — o mesmo dado, agora pela lente de ameaça, não de latência.

### D.7. Senso Crítico

_a preencher._

### D.8. Participantes & Comprobatórios
...
Consolidadas em [Referências](6.Referencias.md) — em especial R10 (Chikofsky & Cross), R11 (Pressman & Maxim), R05 (BPMN 2.0), R08 (HLS/LL-HLS) e R12 (arquitetura de streaming ao vivo).
...
| 1.1 | 23/08/2026 | Definição do fluxo selecionado (Assistir Transmissão ao Vivo) e da plataforma analisada, a partir da Ata 02 | [SubEquipe_01](../../../Projeto/Atas/Atas_Sg1/ata-S1-01-2026-08-23.md) | Lucas Andrade Zanetti |
| 1.2 | 25/08/2026 | Inclusão das hipóteses herdadas do Rich Picture v1 (§3.1), a verificar no FOCO_02 | [SubEquipe_01](../../../Projeto/Atas/Atas_Sg1/ata-S1-02-2026-08-25.md#participantes) | Lucas Andrade Zanetti |
| 1.3 | 27/08/2026 | Execução da engenharia reversa: fundamentação teórica, protocolo reprodutível, seis evidências anonimizadas, 10 atores, 11 atividades/decisões, 14 regras inferidas com grau de confiança, veredito das hipóteses do FOCO_01 e senso crítico | Lucas Andrade Zanetti | _(pendente)_ |
| 1.4 | 27/08/2026 | Narrativa do processo painel a painel (§5.1) com seis figuras em estilo DevTools reconstruídas das evidências; camada de suposições checadas contra documentação e IA (§5.2, com caixas de diálogo A–C); troca dos emojis decorativos e dos vereditos por ícones SVG | Lucas Andrade Zanetti | _(pendente)_ |
| 1.5 | 27/08/2026 | Registro explícito de que o gerador das figuras do DevTools (`figuras_devtools.py`, `gerar_figuras.sh`) foi escrito por IA generativa, para não expor a plataforma analisada e para baratear a regeração do material | Lucas Andrade Zanetti | _(pendente)_ |
