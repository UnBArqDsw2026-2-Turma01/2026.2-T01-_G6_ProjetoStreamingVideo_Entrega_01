...
| -- | -- | -- |
| 3.1 - `PlaybackAccessToken` via GraphQL | Sustenta as tarefas 1.2 e 1.3: antes de obter a playlist, o cliente envia uma operação GraphQL de autorização e recebe os campos equivalentes a `signature` e `value`. A persisted query com hash SHA-256 também aparece em implementações públicas. | Evidência de engenharia reversa. O formato `{sig, token}` é equivalente ao observado, mas não comprova que o token seja internamente um selo HMAC-SHA256. |
| 3.2 - Playlist HLS `.m3u8` pelo Usher | Sustenta a obtenção da master playlist pelo cliente, com `sig` e `token`, antes do início do download dos segmentos de vídeo. | HLS é confirmado oficialmente; o domínio `usher.ttvnw.net` e os parâmetros foram observados por engenharia reversa pública. |
| 3.3 - Múltiplas qualidades e ABR | Sustenta a tarefa 1.4, em que o cliente escolhe automaticamente uma variante de qualidade de acordo com a largura de banda. | Confirmada oficialmente. As faixas de 160p a 1080p aparecem em ladders públicas, mas a implementação especificamente por `HLS.js` não foi comprovada. |
| 3.4 - HLS segmentado | Sustenta a tarefa 1.5, na qual o cliente baixa e armazena temporariamente segmentos de vídeo para manter o buffer de reprodução. | Parcialmente confirmada. O uso de HLS segmentado é sustentado, mas segmentos sempre de 2 segundos, pacotes de 188 bytes e PCR de 27 MHz não foram comprovados como configuração específica da plataforma. |
| 6.1 - Anúncios integrados ao playback | Relacionamos metadados `stitched-ad` encontrados em manifestos ao recebimento de anúncios durante a reprodução no cliente. | Evidência de engenharia reversa. Não comprovamos que todos os anúncios sejam inseridos no fluxo binário usando especificamente ID3 TXXX. |
| 7.2 - Mod View | Sustenta a tarefa 1.7, que representa a interface usada por moderadores para analisar mensagens e tomar decisões. | Confirmada oficialmente como interface central de moderação. |
| 4.1 - Integridade do cliente | Sustenta a presença de mecanismos de integridade no cliente e o envio do cabeçalho `Client-Integrity` em requisições de playback. | Evidência de engenharia reversa. A versão React 18 e a combinação de Kasada com Arkose no mesmo passo não foram comprovadas. |
...
| Evidência | Relação com o print | Classificação e limite |
| -- | -- | -- |
| 3.1 - Operação GraphQL e persisted query | Sustenta o recebimento da consulta pré-compilada, a resolução da operação de playback e a geração dos valores usados na autorização do vídeo. | Evidência de engenharia reversa. A operação e os campos retornados são observáveis, mas a implementação interna do BFF é desconhecida. |
| 4.1 - Integridade e Kasada | Sustenta a tarefa 2.1, pois existem observações públicas de `x-kpsdk-*`, desafios Kasada e proteção anti-automação em partes da plataforma. | Evidência de engenharia reversa. Prova de trabalho em toda requisição, desafio Arkose no mesmo fluxo e decisão de bloqueio exatamente como desenhada são detalhes não comprovados. |
...
| Evidência | Relação com o print | Classificação e limite |
| -- | -- | -- |
| 2.1 - Distribuição por edge | Sustenta a existência de uma camada separada de distribuição por pontos de presença, representada pela pool de borda. | Confirmada oficialmente em nível arquitetural. |
| 3.2 - Usher e playlist `.m3u8` | Sustenta as tarefas 3.1 e 3.2: a borda recebe a requisição com assinatura/token e devolve uma playlist HLS com variantes disponíveis. | HLS é oficial; Usher, `sig` e `token` são sustentados por engenharia reversa pública. A validação HMAC em O(1) não foi comprovada. |
| 3.3 - Variantes HLS | Relacionamos a lista de resoluções devolvida pela borda com as múltiplas variantes produzidas pelo pipeline de transcodificação. | Confirmada oficialmente, com disponibilidade de qualidades variando por transmissão e modo de broadcast. |
| 3.4 - Entrega de segmentos | Sustenta a entrega segmentada mostrada entre a borda e o cliente. | Parcialmente confirmada. Mantemos HLS segmentado como fato sustentado e tratamos duração exata, MPEG-TS de 188 bytes e PCR como detalhes que exigiriam captura de tráfego própria. |
| 6.1 - Conteúdo publicitário `stitched` | Metadados de anúncios observados em manifestos indicam que a camada de distribuição pode entregar conteúdo publicitário integrado ao playback. | Evidência de engenharia reversa; o mecanismo binário exato de inserção não foi comprovado. |
...
| Evidência | Relação com o print | Classificação e limite |
| -- | -- | -- |
| 5.1 - RTMP e stream key | Sustenta as tarefas 4.1 e 4.2: encoders como OBS enviam vídeo para um servidor de ingestão pela estrutura `rtmp://<ingest-server>/app/<stream-key>`, e a stream key identifica e autoriza a transmissão. | Confirmada oficialmente. RTMP é o protocolo documentado; SRT AES na porta 443 como caminho padrão não foi sustentado pelas fontes consultadas. |
| 5.2 - RTMP para múltiplas variantes HLS | Sustenta as tarefas 4.7 a 4.9, que representam criação de variantes, inserção de metadados e empacotamento para distribuição. | Confirmada oficialmente quanto ao recebimento RTMP, conversão para HLS e geração de vários bitrates e resoluções. |
| 5.3 - Transcoder próprio | Sustenta a existência de uma fábrica de vídeo com stack de transcodificação própria, capaz de produzir várias variantes em um processo. | Confirmada oficialmente para o `TwitchTranscoder`; o nome interno `Weaver` usado no print não foi comprovado. |
| 14.5 - Audible Magic | Relacionamos a detecção de áudio protegido ao ecossistema de processamento de vídeo, mas a evidência pública aponta principalmente para VODs e clips. | Parceria historicamente confirmada. Audible Magic decidindo em tempo real se uma live continua não é sustentado e deve ser entendido como hipótese do modelo. |
| 15.1 - Twitch DJ Program | Sustenta a existência de participação no programa e de um catálogo de músicas autorizadas ou restritas. | Confirmada oficialmente no nível de regras do produto. Os campos internos e o fingerprint acústico ao vivo exatamente como modelados não foram comprovados. |
...
| Evidência | Relação com o print | Classificação e limite |
| -- | -- | -- |
| 7.1 - IRC modificado e WebSocket | Sustenta a conexão em `wss://irc-ws.chat.twitch.tv:443`, o servidor TMI, mensagens `PRIVMSG`, tags, badges e o uso de PING/PONG para manter a sessão. | Confirmada oficialmente. |
| 8.1 - AutoMod e mensagens retidas | Sustenta as tarefas 5.2 a 5.6: análise automática, retenção para revisão, decisão de permitir ou negar e aplicação de timeout/ban. | Confirmada oficialmente por APIs de moderação e pelos eventos `automod.message.hold` e `automod.message.update`. |
| 8.2 - Detecção de evasão de banimento | Sustenta a existência de classificação de usuário suspeito e do estado `DETECTED_BAN_EVADER`. | A funcionalidade é confirmada, mas fingerprint de hardware, subnet IP e o algoritmo específico desenhado na tarefa 5.3 não são públicos. |
...
| Evidência | Relação com o print | Classificação e limite |
| -- | -- | -- |
| 9.1 - Pipeline Spade | Sustenta a existência do Spade como pipeline de estatísticas e sua ingestão de milhões de eventos por segundo em um data lake. | Confirmada oficialmente. O endpoint `POST /track`, payload obrigatoriamente Base64, métrica de foco da janela e execução direta de anti-viewbot pelo Spade não foram comprovados. |
...
| Evidência | Relação com o print | Classificação e limite |
| -- | -- | -- |
| 10.1 - OAuth 2.0 | Sustenta o login e a emissão de access tokens de usuário ou aplicação. | Confirmada oficialmente. |
| 10.2 - OIDC e JWT RS256 | Sustenta o uso de OpenID Connect e de ID token JWT assinado com RS256. | Confirmada para ID tokens OIDC. Não podemos generalizar que todo access token da plataforma seja JWT RS256. |
| 10.3 - Refresh e revogação | Sustenta as tarefas de renovação e revogação de tokens e o encerramento de sessões. | O conceito é confirmado; access token fixo de 4 horas, Refresh Token Rotation universal e blacklist em Redis Cluster não foram comprovados. |
| 10.4 - Segundo fator | Sustenta o gateway de 2FA e a verificação por aplicativo autenticador/TOTP ou SMS. | Confirmada oficialmente. WebAuthn/passkeys não foram comprovados pelas fontes reunidas. |
| 10.5 - PKCE | O nome da pool menciona PKCE, mas a documentação consultada lista outros fluxos OAuth sem apresentar `code_challenge`. | Não comprovada e possivelmente incorreta no modelo; usamos OAuth 2.0/OIDC como parte sustentada. |
...
| Evidência | Relação com o print | Classificação e limite |
| -- | -- | -- |
| 11.1 - Domínios de dados | As APIs comprovam a existência de dados de usuários, canais, streams, follows, bloqueios, subscriptions, Bits, emotes, VODs, clips, moderação, Guest Star, tags e classificação de conteúdo. Esses domínios sustentam os oito agrupamentos conceituais mostrados no print. | Parcialmente confirmada. As fontes não revelam o schema interno nem comprovam tabelas chamadas `users`, `stream_keys`, `vods`, `held_messages` ou `moderation_strikes`; esses nomes são nosso modelo lógico inferido. |
...
| Evidência | Relação com o print | Classificação e limite |
| -- | -- | -- |
| 12.1 - Follows | Sustenta a tarefa 9.1, pois a API permite listar canais seguidos e seguidores e apresenta o instante `followed_at`. | Confirmada oficialmente. |
| 12.2 - Bloqueio de usuários | Sustenta a tarefa 9.2 e o gateway que impede a interação quando existe bloqueio, por meio das operações de listar, bloquear e desbloquear usuários. | Confirmada oficialmente. |
| 12.3 - Whispers | Sustenta a tarefa 9.3, pois existe endpoint público para envio de mensagem privada. | A funcionalidade é confirmada; roteamento interno especificamente P2P via TMI não foi comprovado. |
...
| Evidência | Relação com o print | Classificação e limite |
| -- | -- | -- |
| 13.1 - Tiers e Prime | Sustenta a seleção de subscription Tier 1, Tier 2, Tier 3 ou Prime representada no início do fluxo comercial. | Confirmada oficialmente. |
| 13.2 - Emotes, badges e benefício sem anúncios | Sustenta a ativação de benefícios do assinante e o carregamento de emotes exclusivos após a aprovação da compra. | Confirmada oficialmente, observando que a visualização sem anúncios depende da configuração do criador. |
| 13.3 - Bits e Cheers | Sustenta a compra e o uso de Bits, Cheers e Cheermotes no fluxo de monetização. | Confirmada oficialmente. |
| 13.4 - Hype Train | Sustenta o disparo de Hype Train por picos de subscriptions e Bits. | Confirmada oficialmente. A estrutura exata de persistência financeira não é revelada. |
...
| Evidência | Relação com o print | Classificação e limite |
| -- | -- | -- |
| 14.1 - Armazenamento de transmissões | Sustenta o gateway 11.1, pois o criador precisa habilitar o armazenamento para que as lives sejam gravadas e salvas como VOD. | Confirmada oficialmente. |
| 14.2 - Clips de 5 a 60 segundos | Sustenta a tarefa 11.2 e a relação do clip com `video_id` e `vod_offset`. | Confirmada oficialmente. |
| 14.3 - Highlights e markers | Sustenta a tarefa 11.3, pois stream markers permitem retornar a momentos específicos e criar destaques posteriormente. | Confirmada oficialmente. |
| 14.4 - Detecção e mute de áudio protegido | Sustenta a verificação de áudio e o silenciamento automático de trechos protegidos em VODs. | Confirmada oficialmente para conteúdo gravado. |
| 14.5 - Audible Magic | Sustenta historicamente a tecnologia de reconhecimento de áudio associada ao processamento de VOD. | Parceria confirmada; a implementação atual pode evoluir e não comprova corte automático de live durante a ingestão. |
| 3.2 - Playlist HLS de VOD | Sustenta a distribuição do VOD por playlist `.m3u8`, com padrão público de URL `usher.ttvnw.net/vod/<id>.m3u8`. | Evidência de engenharia reversa somada à confirmação oficial do uso de HLS. |
...
| Evidência | Relação com o print | Classificação e limite |
| -- | -- | -- |
| 17.1 - Políticas para conteúdo sensível | Sustenta os tipos de violação considerados no fluxo, como nudez, conteúdo sexual, violência extrema e gore, além da possibilidade de sanções por descumprimento. | Confirmada oficialmente no nível das regras e do enforcement. |
| 17.2 - Machine learning em segurança | Sustenta que a plataforma emprega modelos de ML em tarefas de segurança, como revisão de emotes e detecção de nomes ofensivos. | Parcialmente confirmada. Não encontramos prova de amostragem de toda live a cada 2-5 segundos, classificadores de nudez/gore/armas, `KILL STREAM RTMP` automático, fila humana com SLA menor que 30 segundos ou pipeline de computer vision exatamente como desenhado. Tratamos toda a pool como hipótese arquitetural. |
