...
```mermaid
graph TD
    A[Incidente ou Conflito Identificado] --> B[Nível 1: Diálogo Direto & Alinhamento Construtivo]
    B -->|Resolvido| F[Normalização das Atividades]
    B -->|Persistência / Gravidade Moderada| C[Nível 2: Mediação Interna da Equipe]
    C -->|Acordo / Adequação| F
    C -->|Não resolvido / Gravidade Alta| D[Nível 3: Escalonamento para Docência e Monitoria]
    D --> E[Orientação e Medidas Institucionais FGA/UnB]
```
